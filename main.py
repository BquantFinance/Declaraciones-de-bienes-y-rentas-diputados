...
# FINAL, COMPLETE, AND AGGRESSIVE apply_css FUNCTION (COPY-PASTE READY)
def apply_css():
    st.markdown("""
    <style>
        ...
        /* --- CRITICAL FIX FOR GLASSMORPHISM --- */
        /* This makes the main content area and widget wrappers transparent */
        .main .block-container, 
        div[data-testid="stVerticalBlock"] div[data-testid="stVerticalBlock"] > div:nth-of-type(1),
        .st-emotion-cache-1y4p8pa, 
        .st-emotion-cache-1v0mbdj, 
        .st-emotion-cache-1d3w3a3 {
            background-color: transparent !important;
        }

        /* --- WIDGET STYLING WITH AGGRESSIVE OVERRIDES --- */
        ...
        div[data-testid="stAlert"] > div[data-baseweb^="alert"] { background-color: transparent !important; }
        div[data-testid="stAlert"][data-baseweb="alert-success"] { background-image: linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(16, 185, 129, 0.05) 100%) !important; border-left-color: #10b981 !important; }
        div[data-testid="stAlert"][data-baseweb="alert-warning"] { background-image: linear-gradient(135deg, rgba(245, 158, 11, 0.1) 0%, rgba(245, 158, 11, 0.05) 100%) !important; border-left-color: #f59e0b !important; }
        div[data-testid="stAlert"][data-baseweb="alert-error"] { background-image: linear-gradient(135deg, rgba(239, 68, 68, 0.1) 0%, rgba(239, 68, 68, 0.05) 100%) !important; border-left-color: #ef4444 !important; }
        div[data-testid="stAlert"][data-baseweb="alert-info"] { background-image: linear-gradient(135deg, rgba(59, 130, 246, 0.1) 0%, rgba(59, 130, 246, 0.05) 100%) !important; border-left-color: #3b82f6 !important; }
        ...
        /* --- ALL OTHER ORIGINAL STYLES (UNCHANGED) --- */
        
        .stApp::before { content: ''; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 20% 50%, rgba(102, 126, 234, 0.1) 0%, transparent 50%), radial-gradient(circle at 80% 80%, rgba(118, 75, 162, 0.1) 0%, transparent 50%); pointer-events: none; z-index: 1; }
        .disclaimer-container { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 2px solid rgba(255, 255, 255, 0.1); border-radius: 20px; padding: 3rem; margin: 2rem auto; max-width: 800px; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3), inset 0 0 30px rgba(255, 255, 255, 0.05); animation: fadeInUp 0.8s ease-out; }
        @keyframes fadeInUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
        .disclaimer-title { font-size: 2rem; font-weight: 700; background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); background-clip: text; -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-align: center; margin-bottom: 2rem; text-shadow: 0 0 30px rgba(239, 68, 68, 0.3); }
        .disclaimer-text { color: #e2e8f0; font-size: 1.1rem; line-height: 1.8; text-align: justify; margin-bottom: 2rem; }
        .disclaimer-text strong { color: #fbbf24; font-weight: 600; }
        h1 { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); background-clip: text; -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 700 !important; font-size: 2.5rem !important; margin-bottom: 0.5rem !important; filter: drop-shadow(0 0 20px rgba(102, 126, 234, 0.3)); transition: all 0.3s ease; }
        h1:hover { filter: drop-shadow(0 0 30px rgba(102, 126, 234, 0.5)); transform: scale(1.02); }
        h2 { color: #ffffff !important; font-weight: 600 !important; font-size: 1.8rem !important; text-shadow: 0 2px 10px rgba(0, 0, 0, 0.3); }
        h3 { color: #e2e8f0 !important; font-weight: 500 !important; transition: color 0.3s ease; }
        h3:hover { color: #667eea !important; text-shadow: 0 0 10px rgba(102, 126, 234, 0.5); }
        h4, h5 { color: #cbd5e1 !important; font-weight: 500 !important; }
        .info-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.2rem; margin: 1.5rem 0; }
        .info-item { background: linear-gradient(135deg, rgba(102, 126, 234, 0.1) 0%, rgba(118, 75, 162, 0.05) 100%); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); padding: 1rem; border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.2); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); position: relative; overflow: hidden; }
        .info-item:hover { transform: translateY(-5px) scale(1.02); border-color: rgba(102, 126, 234, 0.4); box-shadow: 0 12px 40px rgba(102, 126, 234, 0.3), inset 0 0 20px rgba(255, 255, 255, 0.05); background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.08) 100%); }
        .info-label { font-size: 0.8rem; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 0.5rem; font-weight: 600; opacity: 0.8; }
        .info-value { font-size: 1.1rem; color: #ffffff; font-weight: 500; }
        div[data-testid="metric-container"] { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.18); padding: 1.2rem; border-radius: 16px; box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.37); transition: all 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94); }
        div[data-testid="metric-container"]:hover { transform: translateY(-8px) scale(1.05); box-shadow: 0 20px 50px rgba(102, 126, 234, 0.4); }
        div[data-testid="stTabs"] [data-baseweb="tab-list"] { background: rgba(255, 255, 255, 0.03); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.1); }
        ...
        .social-pill { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border: 2px solid rgba(255, 255, 255, 0.1); width: 50px; height: 50px; border-radius: 50%; font-size: 1.5rem; text-decoration: none; display: inline-flex; align-items: center; justify-content: center; transition: all 0.3s cubic-bezier(0.68, -0.55, 0.265, 1.55); box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2); position: relative; overflow: hidden; }
        .social-pill:hover { background: rgba(102, 126, 234, 0.2); transform: scale(1.2) rotate(10deg); box-shadow: 0 8px 25px rgba(102, 126, 234, 0.4); border-color: rgba(102, 126, 234, 0.5); }
        ::-webkit-scrollbar { width: 12px; }
        ::-webkit-scrollbar-track { background: rgba(255, 255, 255, 0.02); border-radius: 10px; }
        ::-webkit-scrollbar-thumb { background: linear-gradient(135deg, rgba(102, 126, 234, 0.4) 0%, rgba(118, 75, 162, 0.4) 100%); border-radius: 10px; border: 2px solid rgba(255, 255, 255, 0.1); }
