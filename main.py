...
def get_deputy_photo(deputy_index):
    """Get deputy photo from fotos_diputados folder"""
    ...
    try:
        ...
        for entry in json_data:
            if 'data' in entry and entry['data']:
                ...
                # Get deputy index from source file
                source_file = entry.get('source_file', '')
                deputy_idx = 1
                if 'deputy_' in source_file:
                    try:
                        deputy_idx = int(source_file.split('deputy_')[1].split('_')[0])
                    except:
                        pass
                
                # Override with cod_parlamentario if available
                csv_info = csv_mapping.get(name, {})
                if csv_info.get('cod_parlamentario'):
                    try:
                        deputy_idx = int(csv_info['cod_parlamentario'])
                    except:
                        pass
                
                processed_data.append({
                    'deputy_index': deputy_idx,
                    'Nombre': name,
...
if not st.session_state.disclaimer_accepted:
    ...
    **Esta aplicación constituye una herramienta independiente de análisis y visualización de información pública** 
    disponible en el portal oficial del Congreso de los Diputados.
    """)
        ...
        if not comparison_mode:
            ...
            if names:
                ...
                if selected_name:
                    ...
                    person_idx = person_row['deputy_index']
                    ...
                    with col1:
                        ...
                        party_logo_path = get_party_logo(party_name)
                    ...
                    with col2:
                        ...
                        if twitter:
                            if 'twitter.com/' in twitter:
                                ...
                                handle = twitter
                            social_links += f'<a href="https://twitter.com/{handle}" target="_blank" style="text-decoration: none; margin-right: 10px;">🐦</a>'
                        ...
                        if instagram:
                            if 'instagram.com/' in instagram:
                                ...
                                handle = instagram
                            social_links += f'<a href="https://instagram.com/{handle}" target="_blank" style="text-decoration: none; margin-right: 10px;">📷</a>'
                        
                        facebook = person_row.get('Facebook', '')
                        if facebook:
                            social_links += f'<a href="{facebook}" target="_blank" style="text-decoration: none; margin-right: 10px;">📘</a>'
                        
                        website = person_row.get('Website', '')
                        if website:
                            social_links += f'<a href="{website}" target="_blank" style="text-decoration: none;">🌐</a>'
                        ...
                        <div style="padding-left: 15px;">
                            ...
                            {f'<div style="margin: 10px 0; font-size: 1.5rem;">{social_links}</div>' if social_links else ''}
                            ...
                            <p style="color: rgba(224,224,224,0.8); margin: 5px 0;">
                                🎯 <strong>{party_name if party_name else 'Sin partido'}</strong>
                            </p>
                            <p style="color: rgba(224,224,224,0.8); margin: 5px 0;">
                    ...
                    with tab2:
                        ...
                        if bienes.get('inmuebles_urbanos'):
                            ...
                            for prop in bienes['inmuebles_urbanos']:
                                st.markdown(f"""
                                <div class="info-card">
                                    • <strong>{prop.get('clase', 'Inmueble')}</strong> - {prop.get('situacion', '')}
                                </div>
                                """, unsafe_allow_html=True)
                        ...
                        if deudas:
                            for deuda in deudas:
                                ...
                                <div class="info-card">
                                    ...
                                    <br>Saldo: <strong>{deuda.get('saldo_pendiente', 0)}</strong>
                                </div>
                                """, unsafe_allow_html=True)
            ...
            if selected_name1 and selected_name2:
                ...
                with col1:
                    st.metric(selected_name1[:25], "")
                
                with col2:
                    st.markdown("<p style='text-align: center;'>📊</p>", unsafe_allow_html=True)
                
                with col3:
                    st.metric(selected_name2[:25], "")
    
    with main_tab2:
        st.markdown("### 📊 Tabla de Datos Completa")
        
        search = st.text_input("🔍 Buscar:", placeholder="Nombre, circunscripción...")
        ...
        display_df = display_df.drop('deputy_index', axis=1, errors='ignore')
        
        st.dataframe(display_df, use_container_width=True, hide_index=True, height=600)
        
        csv = display_df.to_csv(index=False)
        st.download_button("⬇️ Descargar CSV", data=csv, file_name='datos.csv', mime='text/csv')
